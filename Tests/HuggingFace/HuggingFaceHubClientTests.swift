...
class HuggingFaceHubClientTests: XCTestCase {
    override func setUp() {
    ...
    func testFilenameRetrieval() async {
        do {
            let filenames = try await HuggingFace.Hub.getFilenames(from: "coreml-projects/Llama-2-7b-chat-coreml")
    ...
    func testFilenameRetrievalWithGlob() async {
        do {
            try await {
                let filenames = try await HuggingFace.Hub.getFilenames(from: "coreml-projects/Llama-2-7b-chat-coreml", matching: "*.json")
                XCTAssertEqual(
                    Set(filenames),
                    Set([
    ...
    func testFilenameRetrievalFromDirectories() async {
        do {
            ...
            let filenames = try await HuggingFace.Hub.getFilenames(from: "coreml-projects/Llama-2-7b-chat-coreml", matching: "*.mlpackage/*")
            XCTAssertEqual(
                Set(filenames),
                Set([
                    "llama-2-7b-chat.mlpackage/Manifest.json",
    ...
    func testFilenameRetrievalWithMultiplePatterns() async {
        do {
            ...
            let filenames = try await HuggingFace.Hub.getFilenames(from: "coreml-projects/Llama-2-7b-chat-coreml", matching: patterns)
            XCTAssertEqual(
                Set(filenames),
...
class SnapshotDownloadTests: XCTestCase {
    ...
    func testDownload() async throws {
        let client = HuggingFace.Hub.Client(downloadBase: downloadDestination)
        var lastProgress: Progress? = nil
        let downloadedTo = try await client.snapshot(from: repo, matching: "*.json") { progress in
    ...
    func testDownloadInBackground() async throws {
        let client = HuggingFace.Hub.Client(downloadBase: downloadDestination, useBackgroundSession: true)
        var lastProgress: Progress? = nil
        let downloadedTo = try await client.snapshot(from: repo, matching: "llama-2-7b-chat.mlpackage/Data/com.apple.CoreML/Metadata.json") { progress in
            print("Total Progress: \(progress.fractionCompleted)")
            print("Files Completed: \(progress.completedUnitCount) of \(progress.totalUnitCount)")
            lastProgress = progress
        }
    ...
    func testCustomEndpointDownload() async throws {
        let client = HuggingFace.Hub.Client(downloadBase: downloadDestination, endpoint: "https://hf-mirror.com")
        var lastProgress: Progress? = nil
        let downloadedTo = try await client.snapshot(from: repo, matching: "*.json") { progress in
            print("Total Progress: \(progress.fractionCompleted)")
            print("Files Completed: \(progress.completedUnitCount) of \(progress.totalUnitCount)")
